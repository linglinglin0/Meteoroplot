...
class Axes :
    ...
    def colorbar(self,fig,ax,X,Y,val,level=30):
        ...
        cbar.set_label("纬向风速（m/s））", fontsize=13)
...
def plt_globalwind_monthaverage(data_address1,data_address2):
    ...
    #    , transform=ccrs.PlateCarree())
    ax1.colorbar(fig, ax, wind1.lons, wind1.lats, wind1.data_value)
    c3 = ax.quiver(wind1.lons[::3,::3], wind1.lats[::3,::3], wind1.data_value[::3,::3], wind2.data_value[::3,::3],transform=ccrs.PlateCarree(), scale=400,
                    color=[0, 0, 0])
    ax.quiverkey(c3, 0.98, -0.2, U=10, label='10m/s', color='k')#在图上的x，y坐标位置
    ax1.add_title('wind_global', month='6-8', level=925)
    # plt.savefig('6-8月925hpa平均流场')
    plt.show()
